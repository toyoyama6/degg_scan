...
def run(filepath, run_json, r_point, t_point, icm_ports, deggNameList, deggList, sessionList, portList, hvSetList, thresholdList,
    ...
    tabletop = doInitialize('tabletop', ['tabletop', 'tabletop'], 5011, 6008, [0, 0], [9000, 14000],
                            30000, period, deadtime, [None, None], filepath=filepath,
...
###################################################
    ...
    deggNameList, deggList, sessionList, portList, hvSetList, thresholdList, baselineFileList, baselineList = configureBaselines(
        run_file=run_json, n_jobs=n_jobs, fStrength=fStrength, tSleep=tSleep,
        overwrite=overwrite, key=key, ignoreList=ignoreList)
##########################################################

    print(f"thresholdList = {thresholdList}")
    # measuring
    for theta_point in theta_scan_points:
        ...
        for event_num, r_point in enumerate(r_scan_points):
            ...
            ##take DEgg data
            print("r_point =", r_point, "theta_point =", theta_point)
            run(dir_sig, run_json, r_point, theta_point, icm_ports, deggNameList, deggList, sessionList, 
...
def daq_wrapper(run_json, comment, meas_type):
    ...
    #theta_scan_points = theta_scan_points[52:]
