...
dirs = glob.glob('./data/splitfiber/*/')

dirs.sort()
dirs.remove('./data/splitfiber/trash/')
...
for i in tqdm(dirs):
    ...
    for k in dfs:
        ...
        for t in range(len(df)):
            ...
            inte_value = integrate.simps(y[1600:1700], dt[1600:1700])

            charge_list.append((inte_value/50)*10**12)

            if(DEBUG==True):
                plt.figure()
                plt.plot(dt[1600:1700], y[1600:1700])
                plt.show()
...
plt.legend()
plt.savefig('./graph/splitfiber/test.png', bbox_inches='tight')