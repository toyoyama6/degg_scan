...
def run_timing(run_file, comment, n_jobs, fStrength = 1,
    ...
    #n_rapcals = 10 ##number of repeats

    #n_rapcals = 180 ##number of repeats
    n_rapcals = 10 ##number of repeats
    ...
    if len(ports) - len(ignoreList) != len(deggBatches):
        print(f'Number of ports {len(ports)}')
        print(f'Ignore List {len(ignoreList)}')
        print(f'DEgg Batches {len(deggBatches)}')
