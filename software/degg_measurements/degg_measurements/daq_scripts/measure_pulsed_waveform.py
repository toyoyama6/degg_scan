...
def measure_pulsed_waveform(run_json, comment, strength0, strength1, n_jobs, mode=None):
    if mode is None:
    ...
    dirname = create_save_dir("/home/scanbox/initialtest/peak_height/", measurement_type=measurement_type)

    ##setup filter wheel
    strength = 1

    if mode == 'droop':
        bursts = [100, 150] ##10%, 50%
        if float(strength) == 0.10:
            ...
            raise ValueError("No valid strength given for droop mode!")
    if mode == 'double':
        burst_num = 2
...
@click.option('--strength0', '-s0', default=0.05,  type=float)
@click.option('--strength1', '-s1', default=1.0, type=float)
@click.option('--mode', '-m', default='double')
@click.option('--n_jobs', '-j', default=1)
@click.option('--force',  is_flag=True)
def main(run_json, comment, strength0, strength1, n_jobs, mode, force):
    
    measure_pulsed_waveform(run_json, comment, float(strength0), float(strength1), n_jobs, mode)
