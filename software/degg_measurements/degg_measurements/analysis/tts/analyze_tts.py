    ...
    for it0, port in enumerate(df_degg.port.unique()):
        for it1, channel in enumerate([1]):
            ...
            if spe == True:
                ...
                new_mask = make_laser_freq_mask_spe(_df.timestamp.values, rate)
                #if port == 5006 and channel == 0:
                #    #new_mask = make_laser_freq_mask_spe(_df.timestamp.values, _df.charge.values,
                #    #                                    rate, smallOffset=7, pairRange=3000)
                #    new_mask = make_laser_freq_mask(_df.timestamp.values, rate)
                #elif port == 5007 and channel == 0:
                #    #new_mask = make_laser_freq_mask_spe(_df.timestamp.values, _df.charge.values,
                #    #                                    rate, smallOffset=7, pairRange=3000)
                #    new_mask = make_laser_freq_mask(_df.timestamp.values, rate)
                #else:
                #    new_mask = make_laser_freq_mask_spe(_df.timestamp.values, _df.charge.values, rate)
...
def findValidTriggers(df_degg, df_ref):
    ...
    for port in df_degg.port.unique():
        print("port =", port)
        for channel in [1]:
            print(df_degg[df_degg.port==port])
            ...
            # print(_df_s.blockNum.unique()[1:])
            ##only match within blocks
            fig1, ax1 = plt.subplots(tight_layout = True)                                                
            ax1.set_xlabel('block number', fontsize = 18)
            ax1.set_ylabel('offset', fontsize = 18)
...
def calculateTTS(df, plot_dir, laser_freq, silence=False):
    ...
    for port in sorted(df_t.port.unique()):
        ...
        for channel in [1]:
            ...
            bins = np.linspace(np.min(centered_tt), np.max(centered_tt), 200)
            hist, edges = np.histogram(centered_tt, bins=bins)
            p0 = [np.max(hist), np.median(centered_tt), 8e-9]
...
def analysis_wrapper(run_file, measurement_number="latest", verbose=False, cache_file=None,
    ...
    fit_df = calculateTTS(df_matched, plot_dir, rate, silence)
