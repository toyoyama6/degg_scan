...
def main():
    ...
    plt.xlabel("Sampling",ha='right',x=1.0)
    plt.ylabel("Pressure [hPa]",ha='right',y=1.0)
    line = None
    
    ii = 0
    pressures = []
    while (True):
        pressurevalue = session.readPressure()
        temperaturevalue = session.readPressureSensorTemperature()
        pressures.append(pressurevalue)
        #print(pressures)
        xdata = [x for x in range(len(pressures))]
        #print(xdata)
        print('{0} hPa'.format(pressurevalue))
        print('{0} degC'.format(temperaturevalue))
        plt.plot(xdata,pressures,marker='.')
        wfrange = (max(pressures) - min(pressures))
        plt.axis([0, len(pressures), max(pressures) - wfrange * 1.2, min(pressures) + wfrange * 1.2])
        fig.canvas.draw()
